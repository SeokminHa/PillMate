...
async function fetchMfds(endpoint: string, params: Record<string, string>) {
  ...
  if (!rawKey) throw new Error("MFDS_API_KEY is not configured");
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/drug/search", async (req: Request, res: Response) => {
    try {
      ...
      const data = await fetchMfds("DrbEasyDrugInfoService/getDrbEasyDrugList", { itemName: name, pageNo, numOfRows });
      const body = data?.body;
      res.json({ items: body?.items ?? [], totalCount: body?.totalCount ?? 0 });
    } catch (err: any) {
